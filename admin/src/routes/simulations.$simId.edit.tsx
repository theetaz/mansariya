    ...
    <div className="flex flex-1 flex-col gap-4 p-4 lg:p-6">
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl space-y-6">
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Input
                value={routeSearch}
                onChange={(e) => { setRouteSearch(e.target.value); setRouteId(''); }}
        ...
        <div className="rounded-lg border p-4">
          <button type="button" className="flex w-full items-center justify-between" onClick={() => setShowAdvanced(!showAdvanced)}>
            <div>
          ...
          {showAdvanced && (
            <div className="mt-3 grid grid-cols-3 gap-3 border-t pt-3">
              <div><Label className="text-xs">Speed Min (km/h)</Label><Input type="number" value={speedMin} onChange={(e) => setSpeedMin(parseFloat(e.target.value))} /></div>
              <div><Label className="text-xs">Speed Max (km/h)</Label><Input type="number" value={speedMax} onChange={(e) => setSpeedMax(parseFloat(e.target.value))} /></div>
              <div><Label className="text-xs">Ping Interval (sec)</Label><Input type="number" value={pingIntervalSec} onChange={(e) => setPingIntervalSec(parseInt(e.target.value))} /></div>
              <div><Label className="text-xs">Dwell Min (sec)</Label><Input type="number" value={dwellMin} onChange={(e) => setDwellMin(parseInt(e.target.value))} /></div>
              <div><Label className="text-xs">Dwell Max (sec)</Label><Input type="number" value={dwellMax} onChange={(e) => setDwellMax(parseInt(e.target.value))} /></div>
            </div>
          )}
        </div>
        ...
        <div className="space-y-3">
          ...
          {vehicles.map((v, i) => (
            <SimulationVehicleCard key={i} index={i} vehicle={v} onChange={updateVehicle} onRemove={removeVehicle} stops={stops} />
          ))}
        </div>
        ...
        <div className="flex gap-2">
          ...
          <Button type="button" variant="outline" onClick={() => navigate({ to: '/simulations' })}>Cancel</Button>
        </div>
      </form>
    </div>
  );
