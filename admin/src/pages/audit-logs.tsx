...
import { fetchAuditLogs, type AuditEntry, type AuditLogsParams } from "@/lib/api"
import {
  DataTable,
    ...
    cell: ({ row }) =>
      row.original.target_type && row.original.target_id ? (
        <div>
          ...
          <span className="ml-1 text-xs text-muted-foreground">{row.original.target_id.slice(0, 12)}</span>
        </div>
      ) : (
...
export function AuditLogsPage() {
  ...
  const queryParams = useMemo<AuditLogsParams>(() => {
    ...
    for (const f of columnFilters) {
      if (f.id === "action" && f.value) params.action = f.value as string
      if (f.id === "actor_email" && f.value) params.actor_email = f.value as string
      if (f.id === "target_type" && f.value) params.target_type = f.value as string
    }
    return params
    ...
    <div className="flex flex-1 flex-col gap-4 py-4 md:gap-6 md:py-6">
      <div className="px-4 md:px-6">
        ...
        <p className="text-sm text-muted-foreground">
          {total} events recorded. Server-side filtered and paginated.
        </p>
      </div>
